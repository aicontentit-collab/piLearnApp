...
/* ========== Carousel Style Tweaks ========== */
const IGCarouselTweaks = createGlobalStyle`
  .carousel .control-dots { position: absolute; bottom: 10px; left: 0; right: 0; margin: 0 auto; padding: 0; }
  .carousel .control-dots .dot { box-shadow: none; background: rgba(255, 255, 255, 0.85); opacity: 1; }
  .carousel .control-next.control-arrow, .carousel .control-prev.control-arrow { display: none; }
...
function DetailedDescription({
  ...
  return (
    <p style={baseStyle} ref={innerRef}>
      {children}
    </p>
  );
}
...
export default function InstagramMultiImagePost({
  icon, // pass a string like "BsHeartPulse" OR a React element <BS.BsHeartPulse />
  data,
  onLikeChange,
  onBookmarkChange,
  onShare,
  ...
  const aiPrompt = useMemo(() => {
    const count = carouselMetaData.length;
    if (count === 0) return "";
    const layout = `A split image with ${count} panel${
      count > 1 ? "s" : ""
    } arranged in a 1x${count} layout, aspect ratio 1.78:1.`;
    const frames = carouselMetaData
      .map(
        (item, i) =>
          `Frame ${i + 1} shows ${item.alt || "a themed visual scene"}.`
      )
      .join(" ");
    const footer =
      "All illustrations should follow a clean 2.5D art style with soft shadows and depth, set against a background of hex code #3d0f96. No text should appear in the image.";
  ...
  const onToggleChat = () => {
    setShowChat((prev) => {
      ...
      if (next) {
        ...
        setIsTyping(true);
        const answer =
          detailedDescription || aiPrompt || "I don't have more details yet.";
        setTimeout(() => {
          setIsTyping(false);
  ...
  if (!carouselMetaData || carouselMetaData.length === 0) return null;
  ...
  return (
    <Card role="article" aria-label={`${title} – Instagram multi-image post`}>
      ...
      {!showChat ? (
        <Carousel items={carouselMetaData} duration={3000} />
      ) : (
        <ChatArea>
          ...
          <ChatFooter onSubmit={handleSend}>
            ...
            />
            <SendBtn type="submit" aria-label="Send message">
              <IoMdSend size={18} />
            </SendBtn>
          </ChatFooter>
        </ChatArea>
